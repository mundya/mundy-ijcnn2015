...
def plot_stack_bar(results, get_x_function, filter_function, title, x_label, filename):
    # Create single-axis figure
    figure, axis = pylab.subplots(figsize=(8, 6), frameon=False)
    ...
    # Sort tuples and unzip into stackplot friendly form
    filtered_results.sort()
    unzipped_results = list(zip(*filtered_results))
    ...
    axis.bar(bar_x, unzipped_results[1], label="Input filtering",
             width=bar_width, edgecolor="none", color=cols[0])
    axis.bar(bar_x, unzipped_results[2], bottom=unzipped_results[1],
             label="Neuron update", width=bar_width, edgecolor="none",
             color=cols[1])
    axis.bar(bar_x, unzipped_results[3], bottom=unzipped_results[2],
             label="Output", width=bar_width, edgecolor="none", color=cols[2])
