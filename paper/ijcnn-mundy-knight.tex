...
In this section we evaluate the performance of our new simulation scheme by simulating a larger NEF network based around the model of action selection developed by Stewart, Choo and Eliasmith \parencite{}. 
...
To demonstrate how the performance of our new SpiNNaker simulator scales, we simulated this network at two different scales, with 8 element sequences encoded using 16 dimensional semantic pointers and with 16 element sequences encoding using 32 dimensional semantic pointers. Figure~\ref{fig:results/spa-wall-clock} shows We ran both of these simulations for 10s using both the Nengo reference simulator \parencite{Bekolay2014} and our new SpiNNaker simulator and measuire
%58 cores
%113 cores
\ref{fig:results/spa-wall-clock}
  
  \begin{figure}[!t]
    \includegraphics{figures/sequence}
    \caption{Comparing the output of the action selection and a list of 4 semantic pointers, labelled $A$, $B$, $C$ and $D$ - showing sequence behaviour.}
