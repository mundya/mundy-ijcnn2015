...
Using typical tuning curves, this means that neurons fire at an average rate of approximately \SI{100}{\hertz}, meaning that a population of just 100 will be likely to exceed the maximum outgoing spike rate constraint. 
Similarily, because of the synaptic processing constraint, each of these 100 neurons would only be able to process input from 500 other neurons, making it impossible to handle biologically accurate fan-in.
...
Neural models built using the principals of the NEF may be implemented in any programming language, however, a standard Python library for building and simulating these models has been developed. Nengo \parencite{Bekolay2014} provides several core components which may be used to specify models for simulation:
  ...
  \begin{figure*}
    ...
    \subfloat[Communication channel]{
        \label{fig:background/comms-channel}
        \includestandalone[width=\textwidth]{figures/comms_channel_map}
      }
    \end{minipage}
    \hfil
    ...
    \end{minipage}
    \caption{A communication channel and simplified-SpiNNaker instantiation (a), and a comparison of the (b) spike based algorithm and the (c) value based algorithm that we present in this paper.}
  \end{figure*}
...
In this section, we use this profiling tool to analyse how the CPU usage of the ensembles that make up the communication channel network described in \S\ref{sef:background/constructing} vary with the number of dimensions they are representing and the number of neurons they use to do so. Figures~\ref{fig:results/comm-channel-cpu-16d}~and~\ref{fig:results/comm-channel-cpu-100n} show how 
  ...
  \begin{figure}[!t]
    ...
    \caption{Average percentange of 1ms simulation time step spent in different phases of our simulation algorithm when simulating a communication channel with 100 neurons per-population and varying numbers of dimensions}
    % **TODO** subfigures with most of this caption shared between them would probably be better
