...
To avoid this \citeauthor*{Galluppi2012}\ suggested transmitting factored weight matrices and performing their multiplication on SpiNNaker.
  ...
  In particular storing the synaptic matrices for these networks requires large amounts of memory and high firing rates and dense neural connectivity exceed the computational resources available to a SpiNNaker core simulating a model in biological real-time.
  ...
  This proposed scheme requires around 90\% less memory in many cases and is able to simulate many more neurons per core than would otherwise be possible -- in some up to 2000 neurons per core, double the SpiNNaker architectural target.
