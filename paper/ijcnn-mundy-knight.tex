...
To avoid this \citeauthor*{Galluppi2012}\ suggested transmitting factored weight matrices and performing their multiplication on SpiNNaker.
  ...
  However, while an ATI Radeon HD 7970 GPU can simulate around \num{500000000} neurons in real-time~\parencite{Bekolay2014} -- comparable to our approach running on a 48 chip SpiNNaker system -- scaling beyond one GPU is likely to be constrained by PCI bus bandwidth. Furthermore, GPUs can be expected to consume significantly more power~\parencite{Wang2010} than an equivalent SpiNNaker system~\parencite{stromatias2013power}.
