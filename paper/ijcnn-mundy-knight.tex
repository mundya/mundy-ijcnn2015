...
All data required by the SpiNNaker machine during simulation must be transmitted to it across ethernet. This means that if more data is required on the machine, more time is required to prepare it for simulation.
...
In the communication channel example shown in \figurename~\ref{fig:background/comms-channel}, this means that each neuron fires at an average of \SI{100}{\hertz} meaning that, using 70 neurons per dimensions, the spike rate 
