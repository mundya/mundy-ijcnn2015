...
However, the resultant semantic pointer will not  be exactly \semanticpointer{MOUSE} -- to extract the $\msemanticpointer{MOUSE}$ semantic pointer the result needs to be ``cleaned up''.
...
On the basis of these two parameters (500 dimensions and 70 neurons per dimension) we can determine that functions on these vectors would have to be evaluated using populations of $3.5\times10^4$ neurons -- two of which would be connected using a dense synaptic matrix with $1.225\times10^9$ entries.
...
NEF models typically opt to use fewer neurons firing at rates of up to \SI{400}{\hertz}, meaning that in networks such as the communication channel example discussed in \S~\ref{sef:background/nef}, each neuron fires at an average of around \SI{100}{\hertz}.
If this example is simulated with 70 neurons per dimension, the overall spike rate increases logarithmically with dimensionality as shown in \figurename~\ref{fig:results/network-utilisation}.
This shows that if we consider the 4 dimensional case, the post-synaptic ensemble will receive around 20 incoming spikes per time-step.
Due to the dense connectivity matrices, each of these spikes will then cause a synaptic event for each of the ensembles 280 neurons, resulting in 5600 synaptic events per timestep -- exceeding the limit of 5000 events per timestep found by \textcite{Sharp2013}.
...
%% More exposition here
  ...
  If factored weight matrices are used then both the pre-synaptic core must store \num{17920} values, though the post-synaptic core will not need to store any more data for any further incoming connections.
  Using, as is common, \SI{32}{\bit} to store each value this becomes \SI{4.79}{\mebi\byte} to store the full weight matrix or \SI{70.00}{\kibi\byte} each to store the encoder and decoder (a total of \SI{140.00}{\kibi\byte}).
  ...
  \begin{figure}[!t]
    ...
    \caption{The time spent in different stages of }
