...
\begin{document}
  \begin{tikzpicture}
    ...
    \node [core, right=45pt of A] (B) {\valuecore{B}{5}{2}{2}};
    ...
    \draw [link] ([xshift=-25pt] A_filter.west) -- (A_filter.west);
    \draw [link] (B_decoder.east) -- ++(25pt, 0);

    %% Hide the earlier part of A and the latter part of B
    \fill [fill opacity=0.75, white]
    ([xshift=-2.5pt] A_4.west |- A.north west) rectangle ([xshift=-25pt] A.south west);
    \fill [fill opacity=0.75, white]
      (B_4.east |- B.north east) rectangle ([xshift=25pt] B.south east);
  \end{tikzpicture}

  \begin{tikzpicture}
    %% Create the two cores and connect them, mark spike packets should be placed
    \node [core] (A) {\spikescore{A}{4}{3}};
    \node [core, right=45pt of A] (B) {\spikescore{B}{5}{4}};
    \draw [link] (A_mux.north) -- (B_weights)
      node [pos=0.25, spike packet] (sp1) {}
      node [pos=0.40, spike packet] (sp2) {}
      node [pos=0.55, spike packet] (sp3) {}
      node [pos=0.70, spike packet] (sp4) {};

    %% Draw extra input and output connections
    \draw [link] ([xshift=-25pt] A_weights.west) -- (A_weights.west);
    \draw [link] (B_mux.north) -- ++(25pt, 0);
