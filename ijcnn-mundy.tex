...
\begin{document}
  ...
  \begin{frame}
    ...
    \begin{columns}[t]
      \begin{column}{.55\textwidth}
        ...
        \SI{2.5}{\hour} compute for \SI{1}{\second} simulation
        \begin{itemize}
          \item Scales badly
      ...
      \begin{column}{.55\textwidth}
        \begin{center}
          \includegraphics[width=.55\textwidth]{figures/spinnaker}

          SpiNNaker
        \end{center}

        Low-power computing platform
        \begin{itemize}
          \item Scalable
          \item Real-time
        \end{itemize}
    ...
    A bad fit? Compute, memory and network load must be managed.
  \end{frame}
  ...
  \begin{frame}{Analysis -- NEF on SpiNNaker}
    ...
    \textbf{One weight-matrix requires \SI{2.28}{\gibi\byte} must be split across cores -- only 120 neurons per core}
  \end{frame}
